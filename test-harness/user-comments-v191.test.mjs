...
test('Ver.216 keeps mention presentation isolated and keeps the comment composer in the existing single-column detail layout', () => {
  ...
  assert.match(interaction, /\.comment-submit-hint-v215\s*\{[\s\S]*?grid-column:1;/,
    'shortcut hint must stay in the existing single composer column');
  assert.doesNotMatch(interaction, /\.comment-submit-hint-v215\s*\{[\s\S]*?grid-column:2\s*\/\s*4;/,
    'shortcut hint must not create implicit extra columns in the narrow detail pane');
  assert.doesNotMatch(interaction, /workflow-mention-shell-v156/);
  assert.ok(interaction.length > read('ui-v165.css').length, 'reply presentation must extend the former reaction-only stylesheet');
});
