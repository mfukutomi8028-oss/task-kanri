...
test('browser emulator suites use demo projects, test rooms, and block production RTDB hosts', () => {
  ...
  assert.match(runner, /inherited dependency writer touches workflowV148 only/);
  assert.match(runner, /current V152 reminder clear can delete a concurrent remote winner/);
  ...
  assert.match(workflowAuditSuite, /concurrent remote winner/);
