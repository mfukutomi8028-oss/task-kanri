...
const ROOM = 'test-date-constraint-ownership-v208';
...
async function bootWithoutStableDateFixes(page) {
  ...
  await page.route(/\/stable-fixes-v108\.js(?:\?.*)?$/i, route => route.fulfill({
    ...
    body: '/* Ver.208 audit: stable-fixes-v108.js intentionally disabled */'
  }));
...
test('date-keyboard alone rejects out-of-range and impossible dates while accepting the upper boundary', async ({ page }) => {
  await bootWithoutStableDateFixes(page);
  ...
  await wrapper.locator('.date-segment-year-v127').fill('10000');
  await expect(wrapper.locator('.date-segment-year-v127')).toHaveValue('1000');
  await expect(source).toHaveValue('');
...
test('date-keyboard alone enforces datetime upper bounds through segmented input', async ({ page }) => {
  await bootWithoutStableDateFixes(page);
...
test('dynamic task start date is adopted on task-dialog open without stable date fixes', async ({ page }) => {
  await bootWithoutStableDateFixes(page);
