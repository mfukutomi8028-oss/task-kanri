...
const ROOM = 'test-stable-version-display-v215';
...
async function boot(page) {
  await page.addInitScript(({ room }) => {
    try {
      ...
      localStorage.setItem(`system-task-tasks:${room}`, JSON.stringify([
        { id: 'hold-v215', status: '保留', assignee: '福冨' },
        { id: 'group-v215', status: '対応中', assignee: 'システム課' }
      ]));
    } catch {}
  ...
  await page.waitForFunction(() => {
    ...
    return version === '215' && document.documentElement.dataset.firstPaintVersion === version;
...
async function expectCanonicalVersion(page) {
  ...
  await expect(display).toHaveText('Ver.215');
  await expect(display).toHaveAttribute('data-release-version', '215');
  await expect(display).toHaveAttribute('title', '現在のバージョン Ver.215');
  await expect(page.locator('.app-version')).toHaveCount(0);
  expect(await page.evaluate(() => window.WORK_BOARD_VERSION)).toBe('215');
}
...
test('manifest and version-display-lock exclusively own initial version display while stable remains presentation-free', async ({ page }) => {
  ...
  await page.evaluate(() => {
    ...
    fixture.id = 'stable-version-fixture-v215';
    fixture.innerHTML = `
      <div class="today-panel"><h4>今日のタスク</h4><div>
        <article class="task-card" data-task-id="hold-v215"></article>
        <article class="task-card" data-task-id="group-v215"></article>
      </div></div>`;
    today.appendChild(fixture);
  });

  await expect(page.locator('#stable-version-fixture-v215 [data-task-id="hold-v215"]')).toHaveAttribute('data-v108-hidden', '');
  await expect(page.locator('#stable-version-fixture-v215 [data-task-id="hold-v215"]')).toBeHidden();
  await expect(page.locator('#stable-version-fixture-v215 [data-task-id="group-v215"]')).toBeVisible();
  await expectCanonicalVersion(page);
});
