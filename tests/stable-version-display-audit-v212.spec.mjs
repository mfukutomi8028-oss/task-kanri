...
const ROOM = 'test-stable-version-display-v214';
  ...
  await page.addInitScript(({ room }) => {
    try {
      ...
      localStorage.setItem(`system-task-tasks:${room}`, JSON.stringify([
        { id: 'hold-v214', status: '保留', assignee: '福冨' },
...
async function expectCanonicalVersion(page) {
  ...
  await expect(display).toHaveText('Ver.214');
  await expect(display).toHaveAttribute('data-release-version', '214');
  await expect(display).toHaveAttribute('title', '現在のバージョン Ver.214');
  await expect(page.locator('.app-version')).toHaveCount(0);
  expect(await page.evaluate(() => window.WORK_BOARD_VERSION)).toBe('214');
}
...
test('manifest and version-display-lock exclusively own initial version display while stable remains presentation-free', async ({ page }) => {
  ...
  await page.evaluate(() => {
    ...
    fixture.id = 'stable-version-fixture-v214';
    fixture.innerHTML = `
      <div class="today-panel"><h4>今日のタスク</h4><div>
        <article class="task-card" data-task-id="hold-v214"></article>
        <article class="task-card" data-task-id="group-v214"></article>
      </div></div>`;
    today.appendChild(fixture);
  });

  await expect(page.locator('#stable-version-fixture-v214 [data-task-id="hold-v214"]')).toHaveAttribute('data-v108-hidden', '');
  await expect(page.locator('#stable-version-fixture-v214 [data-task-id="hold-v214"]')).toBeHidden();
  await expect(page.locator('#stable-version-fixture-v214 [data-task-id="group-v214"]')).toBeVisible();
  await expectCanonicalVersion(page);
});
