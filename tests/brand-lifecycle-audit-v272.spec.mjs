...
const ROOM = 'test-brand-lifecycle-v272';
...
async function installAudit(page) {
  await page.addInitScript(room => {
    ...
    window.__WB_BRAND_LIFECYCLE_AUDIT_V272__ = audit;

    const nativeAppendChild = Node.prototype.appendChild;
    Node.prototype.appendChild = function appendChildAuditV272(child) {
    ...
    Element.prototype.remove = function removeAuditV272() {
      const stack = new Error().stack;
    ...
    Element.prototype.setAttribute = function setAttributeAuditV272(name, value) {
      const stack = new Error().stack;
      if (
        sourceOwned(stack)
    ...
    EventTarget.prototype.addEventListener = function addEventListenerAuditV272(type, listener, options) {
      const stack = new Error().stack;
...
async function snapshot(page) {
  return page.evaluate(() => {
    const audit = window.__WB_BRAND_LIFECYCLE_AUDIT_V272__;
...
test('Ver.272 audit: normal startup performs at least two brand-owned favicon replacement passes', async ({ page }) => {
  await boot(page);
  const state = await snapshot(page);
  console.log('V272_BRAND_STARTUP_METRICS', JSON.stringify(state));

  expect(state.release).toBe('258');
  expect(state.pageshowRegistrations).toBe(1);
  expect(state.iconAdds.length).toBeGreaterThanOrEqual(8);
  expect(state.iconAdds.length % 4).toBe(0);
  expect(state.iconRemoves.length).toBeGreaterThanOrEqual(7);
  expect(state.brandMark).toContain('assets/brand-v184.svg?v=185');
  expect(state.notificationBrand).toBe('185');
});

test('Ver.272 audit: a no-drift pageshow reruns favicon replacement but leaves brand mark and Notification identity unchanged', async ({ page }) => {
  await boot(page);
  ...
  console.log('V272_BRAND_PAGESHOW_NODRIFT_METRICS', JSON.stringify(state));

  expect(state.pageshowCallbacks).toBe(1);
  expect(state.iconAdds).toBe(4);
  expect(state.iconRemoves).toBe(4);
  expect(state.brandMarkWrites).toBe(0);
  expect(state.notificationSame).toBe(true);
  expect(state.notificationBrand).toBe('185');
});

test('Ver.272 audit: pageshow repairs synthetic brand, favicon and Notification drift', async ({ page }) => {
  await boot(page);
  const result = await page.evaluate(async () => {
    const audit = window.__WB_BRAND_LIFECYCLE_AUDIT_V272__;
    const brand = document.querySelector('.brand-mark img');
    brand?.setAttribute('src', 'assets/brand.png?v=synthetic-v272');
    const firstIcon = document.head.querySelector('link[rel~="icon"]');
    if (firstIcon) firstIcon.setAttribute('href', 'assets/brand.png?v=synthetic-v272');
  ...
  console.log('V272_BRAND_PAGESHOW_DRIFT_METRICS', JSON.stringify(result));
  ...
  expect(result.icons.every(href => /assets\/brand-v184\.(?:svg|png)\?v=185$/.test(href))).toBe(true);
  expect(result.notificationBrand).toBe('185');
  expect(result.notificationRewrapped).toBe(true);
});

test('Ver.272 audit: normal navigation does not create brand drift before pageshow', async ({ page }) => {
  await boot(page);
  await page.evaluate(() => window.__WB_BRAND_LIFECYCLE_AUDIT_V272__.reset());
  ...
  const state = await snapshot(page);
  console.log('V272_BRAND_NAVIGATION_METRICS', JSON.stringify(state));
