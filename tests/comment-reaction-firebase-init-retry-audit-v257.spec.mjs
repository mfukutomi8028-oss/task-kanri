...
const ROOM = 'test-comment-reaction-firebase-init-retry-v257';

function taskRecord(id) {
  const now = Date.now();
  return {
    id,
    title: 'Ver.257 Firebase init retry audit',
    description: '', requester: '', assignee: '福冨', status: '対応中', priority: '中', category: 'その他',
    tags: [], dueDate: '', dueTime: '', pinned: false, checklist: [],
    comments: [{ id: 'parent-v257', author: '森井', type: '作業メモ', text: 'init retry parent', createdAt: now - 2000 }],
...
async function openTaskComments(page, taskId) {
  ...
  await expect(page.locator('.activity-comment[data-comment-id="parent-v257"]')).toBeVisible({ timeout: 15_000 });
}
...
test('initializeApp transient failure retries from the same loaded modules without advancing import generation', async ({ page }) => {
  const taskId = 'task-v257-init-retry';
  ...
  await page.addInitScript(({ room, task }) => {
    ...
    window.__WB_V257_CONFIG__ = null;
    window.__WB_V257_REMOTE_TASK__ = structuredClone(task);
    window.__WB_V257_INIT_CALLS__ = 0;
    window.__WB_V257_GET_DB_CALLS__ = 0;
    window.__WB_V257_TX_CALLS__ = 0;
    Object.defineProperty(window, 'firebaseConfig', {
      configurable: true,
      get() { return window.__WB_V257_CONFIG__; },
      set() {}
    });
  ...
  await page.route(/https:\/\/www\.gstatic\.com\/firebasejs\/10\.12\.5\/firebase-app\.js(?:\?wb-retry=\d+)?$/, route => {
    ...
    return route.fulfill({
      ...
      body: [
        ...
        '  globalThis.__WB_V257_INIT_CALLS__ += 1;',
        '  if (globalThis.__WB_V257_INIT_CALLS__ === 1) throw new Error("v257-init-transient");',
        '  const app = { config }; apps.push(app); return app;',
        '}'
      ].join('\n')
    });
  });
  ...
  await page.route(/https:\/\/www\.gstatic\.com\/firebasejs\/10\.12\.5\/firebase-database\.js(?:\?wb-retry=\d+)?$/, route => {
    ...
    return route.fulfill({
      ...
      headers: { 'access-control-allow-origin': '*', 'cache-control': 'no-store' },
      body: [
        'export function getDatabase(app){ globalThis.__WB_V257_GET_DB_CALLS__ += 1; return { app }; }',
        'export function ref(db, path){ return { db, path }; }',
        'export async function runTransaction(target, updater){',
        '  globalThis.__WB_V257_TX_CALLS__ += 1;',
        '  const current = structuredClone(globalThis.__WB_V257_REMOTE_TASK__);',
        '  const next = updater(current);',
        '  if (next === undefined) return { committed: false, snapshot: { val: () => structuredClone(globalThis.__WB_V257_REMOTE_TASK__) } };',
        '  globalThis.__WB_V257_REMOTE_TASK__ = structuredClone(next);',
  ...
  await page.evaluate(() => {
    window.__WB_V257_CONFIG__ = { apiKey: 'v257-init-retry', databaseURL: 'https://example.invalid', projectId: 'v257-audit' };
    const pill = document.getElementById('connectionPill');
    if (pill) pill.textContent = '共同編集ON';
  });

  await clickCurrent(page, '[data-comment-reaction-picker="parent-v257"]');
  const choice = page.locator('.comment-reaction-choice-v165[data-comment-reaction-id="parent-v257"][data-comment-reaction-emoji="👍"]');
  ...
  let runtime = await page.evaluate(() => ({ init: window.__WB_V257_INIT_CALLS__, getDb: window.__WB_V257_GET_DB_CALLS__, tx: window.__WB_V257_TX_CALLS__ }));
  expect(runtime).toEqual({ init: 1, getDb: 0, tx: 0 });
  ...
  runtime = await page.evaluate(() => ({
    init: window.__WB_V257_INIT_CALLS__,
    getDb: window.__WB_V257_GET_DB_CALLS__,
    tx: window.__WB_V257_TX_CALLS__,
    remote: structuredClone(window.__WB_V257_REMOTE_TASK__)
  }));
  expect(runtime.init).toBe(2);
