...
const ROOM = 'test-todo-observer-boundary-v264';
...
async function installAuditBoundary(page) {
  await page.addInitScript(({ room, sources }) => {
    ...
    window.MutationObserver = class MutationObserverAuditV264 {
    ...
    window.requestAnimationFrame = function requestAnimationFrameAuditV264(callback) {
      const source = sourceFromStack(new Error().stack);
...
async function boot(page) {
  ...
  await page.waitForFunction(() => {
    const audit = window.__WB_TODO_OBSERVER_AUDIT_V264__;
...
test('Ver.264 inventory: three ToDo sidecars register four subtree observers', async ({ page }) => {
  await boot(page);
  ...
  for (const entry of audit.entries) {
    expect(entry.observes).toEqual(expect.arrayContaining([
      expect.objectContaining({ childList: true, subtree: true, attributes: false })
    ]));
  }
});
