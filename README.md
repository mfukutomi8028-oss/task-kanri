...
- ボード画面上の状態セクションも、見出し左のハンドルをドラッグして並び替えできるようにしました。
- 以前の上下ボタンで1つずつ移動するUIは表示しないようにしました。
- 並び替え結果はFirebaseへ保存され、同じルーム内で共有されます。
- `style.css?v=10` / `app.js?v=10` / 各アイコン画像 `?v=10` にしてブラウザキャッシュを回避しています。


## v11 修正内容

- v10で発生していた重大なJavaScript不具合を修正しました。
- 原因は、`renderTimeline()` が二重に挿入され、以降の関数が意図せず関数内スコープに入っていたことです。
- その影響で以下が発生していました。
  - Firebase上の既存タスクが表示されない
  - ユーザー選択が未選択のままになる
  - 新しいタスクボタンを押しても反応しない
  - ボード描画が途中で停止する
- Firebase設定やRealtime Database自体は変更していません。
- `style.css?v=11` / `app.js?v=11` / 各アイコン画像 `?v=11` にしてブラウザキャッシュを回避しています。
