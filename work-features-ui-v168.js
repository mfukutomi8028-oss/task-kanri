// Ver.168: UI polish for business memos, start-date guidance, and navigation icons.
(function installWorkFeaturesUiV168() {
  'use strict';
  ...
  function patchStartDateField() {
    ...
    if (!label || label.dataset.v168Polished === 'true') return;
    ...
    label.dataset.v168Polished = 'true';
  }
  ...
  function patchMemoDialog() {
    ...
    if (!row || row.dataset.v168Polished === 'true') return;
  ...
  function patchNavIcons() {
    ...
    if (todoIcon && !todoIcon.src.includes('nav-todo-v168.svg')) {
      todoIcon.src = `assets/nav-todo-v168.svg?v=${VERSION}`;
    }

    const memoIcon = document.querySelector('[data-work-memo-layout] .nav-icon img');
    if (memoIcon && !memoIcon.src.includes('nav-memo-v168.svg')) {
      memoIcon.src = `assets/nav-memo-v168.svg?v=${VERSION}`;
    }
  }
