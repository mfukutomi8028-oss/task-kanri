...
const elements = {
  ...
  importTasksFile: $("importTasksFile"),
  importDialog: $("importDialog"),
  importSummary: $("importSummary"),
  importPreview: $("importPreview"),
  importSkipDuplicates: $("importSkipDuplicates"),
  importAddCategories: $("importAddCategories"),
...
function setupEvents() {
  ...
  elements.importTasks?.addEventListener("click", () => {
    elements.importTasksFile.value = "";
    elements.importTasksFile.click();
  });
  elements.importTasksFile?.addEventListener("change", async () => {
    const file = elements.importTasksFile.files?.[0];
    if (file) await prepareExcelImport(file);
  });
  elements.cancelImport?.addEventListener("click", () => elements.importDialog.close());
  elements.closeImportDialog?.addEventListener("click", () => elements.importDialog.close());
  elements.confirmImport?.addEventListener("click", () => confirmExcelImport());
...
async function prepareExcelImport(file) {
  try {
    const rows = await readExcelLikeFile(file);
    const mapped = rows.map((row, index) => mapExcelRowToTask(row, index)).filter(Boolean);
    if (!mapped.length) return toast("取り込めるタスクが見つかりませんでした", true);

    const categories = uniqueCategories([...state.categories, ...mapped.map(item => item.category)]);
    const newCategories = categories.filter(category => !state.categories.some(existing => normalizeText(existing) === normalizeText(category)));
    const users = uniqueUsers([...state.users, ...mapped.map(item => item.assignee).filter(Boolean)]);
    const newUsers = users.filter(user => !state.users.some(existing => normalizeText(existing) === normalizeText(user)));
    const duplicateCount = mapped.filter(item => isImportDuplicate(item)).length;
    ...
    console.error(error);
    toast(error?.message || "取込ファイルを読み込めませんでした", true);
  }
}
...
async function readExcelLikeFile(file) {
  ...
  if (name.endsWith(".xlsx") || name.endsWith(".xls")) {
    if (!window.XLSX) {
      throw new Error("Excelファイルの読込ライブラリを取得できませんでした。CSVまたはタブ区切りテキストで取り込んでください。");
    }
...
function parseDelimitedText(text) {
  ...
  const table = parseDelimitedTable(clean, delimiter).filter(row => row.some(cell => String(cell).trim()));
  if (table.length < 2) return [];
  const headers = table[0].map(header => normalizeExcelHeader(header));
...
function normalizeExcelHeader(value) {
  const header = String(value || "").normalize("NFKC").replace(/\s+/g, "");
  const map = {
    "タスク": "タスク",
    "件名": "タスク",
    "作業内容": "作業内容",
...
function mapExcelRowToTask(row, index) {
  const taskName = getExcelValue(row, "タスク");
  const work = getExcelValue(row, "作業内容");
  const note = getExcelValue(row, "備考");
  if (!taskName && !work && !note) return null;

  const ratio = getExcelValue(row, "割合");
  const progress = getExcelValue(row, "進捗状況");
  const workAmount = getExcelValue(row, "作業量");
  const startIso = parseExcelDateToIso(getExcelValue(row, "開始日"));
  const dueIso = parseExcelDateToIso(getExcelValue(row, "期日"));
  const createdAt = startIso ? isoDateToTimestamp(startIso, "09:00") : Date.now();
  const done = isExcelCompleteRatio(ratio);
  const completedAt = done ? isoDateToTimestamp(dueIso || startIso || todayISO(), "17:00") : 0;
  const assignee = normalizeImportAssignee(getExcelValue(row, "担当者"));
  const category = inferExcelCategory(taskName, work, note);
  const title = buildExcelImportTitle(taskName, work);
  const description = buildExcelImportDescription({ taskName, work, note, progress, ratio, workAmount, startIso, dueIso });

  return normalizeTask({
    id: generateId(),
    title,
    assignee,
    status: done ? COMPLETED_STATUS : "対応中",
    priority: mapExcelPriority(getExcelValue(row, "優先度")),
    category,
    dueDate: dueIso,
    dueTime: "",
    requester: "",
    tags: ["Excel取込", taskName].filter(Boolean),
    description,
    checklist: [],
    comments: [],
    history: appendHistory([], `Excelからインポートしました。元行：${index + 2}`),
    createdBy: assignee,
    createdAt,
    updatedBy: getCurrentUser(),
    updatedAt: completedAt || createdAt,
    completedAt,
    completedMemo: done ? "Excel取込時点で割合100%のため完了扱い" : "",
    pinned: false,
    recurrence: "none"
  });
}

function getExcelValue(row, key) {
  if (!row) return "";
  if (Object.prototype.hasOwnProperty.call(row, key)) return String(row[key] ?? "").trim();
  const found = Object.keys(row).find(item => normalizeText(item) === normalizeText(key));
  return found ? String(row[found] ?? "").trim() : "";
}

function buildExcelImportTitle(taskName, work) {
  const main = String(taskName || "").trim();
  const sub = String(work || "").trim();
  if (main && sub && normalizeText(main) !== normalizeText(sub)) return `${main}：${sub}`;
  return main || sub || "Excel取込タスク";
}

function buildExcelImportDescription({ taskName, work, note, progress, ratio, workAmount, startIso, dueIso }) {
  const lines = [];
  if (work) lines.push(`作業内容：${work}`);
  if (note) lines.push(`備考：${note}`);
  if (taskName) lines.push(`元タスク名：${taskName}`);
  if (startIso) lines.push(`Excel開始日：${startIso}`);
  if (dueIso) lines.push(`Excel期日：${dueIso}`);
  if (ratio) lines.push(`Excel割合：${ratio}`);
  if (progress && progress !== "#VALUE!") lines.push(`Excel進捗状況：${progress}`);
  if (workAmount) lines.push(`Excel作業量：${workAmount}`);
  return lines.join("\n");
}
...
function isoDateToTimestamp(iso, time = "09:00") {
  const date = new Date(`${iso}T${time}:00`);
  return Number.isNaN(date.getTime()) ? Date.now() : date.getTime();
}

function isExcelCompleteRatio(value) {
  const raw = String(value || "").normalize("NFKC").replace("%", "").trim();
  const number = Number.parseFloat(raw);
  return !Number.isNaN(number) && number >= 100;
}

function mapExcelPriority(value) {
  const number = Number.parseInt(String(value || "").normalize("NFKC").trim(), 10);
  if (Number.isNaN(number)) return "中";
  if (number >= 5) return "緊急";
  if (number >= 4) return "高";
  if (number >= 3) return "中";
  return "低";
}

function normalizeImportAssignee(value) {
  const raw = sanitizeUser(value);
  return raw || getCurrentUser();
}

function inferExcelCategory(taskName, work, note) {
  const text = normalizeText([taskName, work, note].filter(Boolean).join(" "));

  const rules = [
    { category: "Web/HP", terms: ["hp", "web", "wordpress", "サイト", "ホームページ"] },
    { category: "プリンタ", terms: ["プリンタ", "印刷", "複合機", "スキャン", "定着", "トナー", "ドラム", "リコー", "ricoh", "バーコードリーダー"] },
    { category: "ネットワーク", terms: ["ネットワーク", "wifi", "wi-fi", "回線", "ドメイン", "xserver", "ぷらら", "dns", "メール送信", "メール受信"] },
    { category: "アカウント", terms: ["職員登録", "新規職員", "id登録", "アカウント", "メールアドレス", "ユーザー登録", "権限"] },
    { category: "在庫管理", terms: ["在庫管理", "物品", "発注", "用度", "滅菌", "中材"] },
    { category: "電子カルテ", terms: ["電カル", "mall", "ナイス", "nice", "オーダー", "予約枠", "レセプト", "薬剤", "dpc", "様式1", "mla", "ml-a", "検温", "オンライン資格", "オン資", "資格確認"] },
    { category: "PC", terms: ["pc", "端末", "ipad", "タブレット", "smilepod", "キッティング", "修理", "リプレイス", "入替", "環境構築", "outlook", "パソコン"] },
    { category: "Excel/資料", terms: ["資料", "excel", "帳票", "マニュアル", "月報", "日報", "文書", "一覧表", "シート", "vba"] },
    { category: "サーバー/設備", terms: ["サーバ", "server", "sv", "ups", "バックアップ", "防犯カメラ", "精算機", "受付機"] },
    { category: "会議/設営", terms: ["会議", "zoom", "teams", "プロジェクター", "説明会", "来客", "セッティング", "設営", "貸出"] },
    { category: "セキュリティ", terms: ["セキュリティ", "bcp", "ウイルス", "usb"] },
    { category: "業者対応", terms: ["問い合わせ", "問合せ", "業者", "メドレー", "prrism", "アルメックス", "見積", "営業"] }
  ];

  const hit = rules.find(rule => rule.terms.some(term => text.includes(normalizeText(term))));
  return hit ? hit.category : "その他";
}

function isImportDuplicate(task) {
  const key = importTaskKey(task);
  return state.tasks.some(existing => importTaskKey(existing) === key);
}

function importTaskKey(task) {
  return [
...
function renderImportPreview() {
  ...
  const doneCount = data.tasks.filter(task => isCompletedStatus(task.status)).length;
  const activeCount = data.tasks.length - doneCount;
  const newCategoryText = data.newCategories.length ? data.newCategories.map(escapeHtml).join("、") : "なし";
  const newUserText = data.newUsers.length ? data.newUsers.map(escapeHtml).join("、") : "なし";
  const duplicateText = data.duplicateCount ? `${data.duplicateCount}件` : "なし";

  elements.importSummary.innerHTML = `
    <div class="import-summary-grid">
      <article><small>読込ファイル</small><strong>${escapeHtml(data.fileName)}</strong></article>
      <article><small>取込候補</small><strong>${data.tasks.length}件</strong></article>
      <article><small>完了</small><strong>${doneCount}件</strong></article>
      <article><small>対応中</small><strong>${activeCount}件</strong></article>
      <article><small>重複候補</small><strong>${escapeHtml(duplicateText)}</strong></article>
    </div>
    <p class="import-summary-text">新規分類：${newCategoryText}</p>
    <p class="import-summary-text">新規担当者：${newUserText}</p>
  `;

  elements.importPreview.innerHTML = `
    <table>
      <thead><tr><th>状態</th><th>優先度</th><th>分類</th><th>担当</th><th>件名</th><th>期限</th></tr></thead>
      <tbody>
        ${data.tasks.slice(0, 12).map(task => `<tr>
          <td>${escapeHtml(task.status)}</td>
          <td>${escapeHtml(task.priority)}</td>
          <td>${escapeHtml(task.category)}</td>
          <td>${escapeHtml(task.assignee)}</td>
          <td>${escapeHtml(task.title)}</td>
          <td>${escapeHtml(task.dueDate || "期限なし")}</td>
        </tr>`).join("")}
      </tbody>
    </table>
    ${data.tasks.length > 12 ? `<p class="import-more">ほか ${data.tasks.length - 12}件</p>` : ""}
  `;
}

async function confirmExcelImport() {
  const data = state.pendingImport;
  if (!data?.tasks?.length) return toast("取込候補がありません", true);

  elements.confirmImport.disabled = true;
  elements.confirmImport.textContent = "取込中...";

  try {
    if (elements.importAddCategories.checked) {
      state.categories = uniqueCategories([...state.categories, ...data.newCategories]);
    ...
    if (data.newUsers.length) {
      const palette = ["#7c5cff", "#35b66f", "#f08bd8", "#4da3ff", "#ff9f43", "#22c1c3"];
      data.newUsers.forEach((user, index) => {
        if (!state.userColors[user]) state.userColors[user] = palette[index % palette.length];
      });
      setUsers([...state.users, ...data.newUsers], { persist: false, silent: true });
      await saveUserSettings(true);
    }

    if (!getStatusList({ includeTaskOnly: false }).some(status => normalizeText(status) === normalizeText("対応中"))) {
      state.statuses.splice(Math.max(1, state.statuses.length - 1), 0, "対応中");
      await saveStatusSettings(true);
      syncStatusOptions($("taskStatus"));
      syncStatusOptions(elements.statusFilter, true);
    }

    const skipDuplicates = elements.importSkipDuplicates.checked;
    const seen = new Set();
    let imported = 0;
    let skipped = 0;
    ...
    for (const task of data.tasks) {
      const key = importTaskKey(task);
      if (skipDuplicates && (seen.has(key) || isImportDuplicate(task))) {
        skipped += 1;
        continue;
      }
      seen.add(key);
    ...
    localStorage.setItem(taskLayoutKey(), state.taskLayout);
    render();
    toast(`Excel取込完了：${imported}件${skipped ? ` / 重複スキップ：${skipped}件` : ""}`);
  } catch (error) {
    console.error(error);
    toast(error?.message || "取込中にエラーが発生しました", true);
  } finally {
    elements.confirmImport.disabled = false;
    elements.confirmImport.textContent = "取り込む";
  }
}
