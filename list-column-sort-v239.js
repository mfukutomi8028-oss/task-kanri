// Ver.239: list-view column sorting plus clear-to-primary-sort rebuild bridge.
// Primary task sort persistence remains owned by saved-views-v148.js and app.js rendering.
(function installListColumnSortingV239() {
  ...
  function requestPrimarySortRestore() {
    requestAnimationFrame(() => {
      const select = document.querySelector(SORT_SELECT_SELECTOR);
      if (!select) return;
  ...
  document.addEventListener('click', event => {
    ...
    if (clearButton) {
      ...
      writeColumnSort(null);
      // Register the canonical primary-sort rebuild before the local enhancement RAF.
      // This preserves the proven Ver.237 ordering without depending on task UX.
      requestPrimarySortRestore();
      scheduleEnhance();
      return;
    }
  ...
  function handleBaseSortChange(event) {
